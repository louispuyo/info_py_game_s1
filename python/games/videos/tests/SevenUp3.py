...
class Bird(object):  # represents the bird, not the game
    ...
    def detect_image(self, data:dict) -> list:
        LIST = []
        for i in data.keys():
            pattern = re.findall("\d.", i)
            # print(pattern)
            LIST.append(str(pattern).strip('[]'))
        return LIST
    ...
    def rotation_px(self):
        if self.time in self.detect_image((vars(self))):
            if self.char != '':
                self.char =  self.char.strip("\'\'")
                self.int = int(self.char)
                self.current_image = self.__getattribute__(f"image_{self.time}")
        elif self.time < 7:
            self.current_image = self.__getattribute__(f"image_0{self.time}")
        else:
            self.time += 4
            print("ERROR")
    ...
    def rotation_nx(self):
        ...
        if self.char in self.detect_image((vars(self))):
            if self.char != '':
                self.char = self.char.strip("\'\'")
                self.int = int(self.char)
